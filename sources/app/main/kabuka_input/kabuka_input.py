...
def run_kabuka_input(logger, execKinouId, syoriymd, ptskbn, db_connection, db_cursol):

    logger.info('｜▼kabuka_input開始')
    logger.info('｜｜PST区分＝%s', ptskbn)
    ...
    for meigara_mst_row in resultset:
        ...
        if ptskbn == C_Pts.NO:
            insertvalue = getKabukaByPtsNone(logger, syoriymd, meigara_id)
        else:
            insertvalue = getKabukaByPts(logger, syoriymd, meigara_id, ptskbn)

        if insertvalue != "":
            moduleDao.insertTbl(logger, db_connection, db_cursol, 't_kabuka', "" , insertvalue)
            cnt_syori = cnt_syori + 1
        else:
            logger.info('｜｜◇株価情報取得失敗')
        #if cnt_syori >= 1000:
        #    break
...
def getKabukaByPtsNone(logger, syoriymd, meigara_id):
...
def getKabukaByPts(logger, syoriymd, meigara_id, ptskbn):
    kabutan_url = "http://www.morningstar.co.jp/StockInfo/pts/info/" + meigara_id
    ...
    tmp_owarine = soup.find(id="ptsCurrentValue").text
    tmp_yakujyou_cnt = 0
    tmp_jikasougaku = 0

    if tmp_owarine == '-':
        tmp_owarine_before = 0
        tmp_harimarine = 0
        tmp_takane = 0
        tmp_yasune = 0
        tmp_dekidaka = 0
        ...
        tmp_owarine_before_zoukaritu = 0
        tmp_owarine_before_zoukagk = 0

    else:
        tmp_owarine_before = soup.find(id="ptsReferenceValue").text.replace(',', '')
        tmp_harimarine = soup.find(id="ptsBeginValue").text.replace(',', '')
        tmp_takane = soup.find(id="ptsMaxValue").text.replace(',', '')
        tmp_yasune = soup.find(id="ptsMinValue").text.replace(',', '')
        tmp_dekidaka = soup.find(id="ptsTradingVolume").text.replace(',', '')
        tmp_owarine = tmp_owarine.replace(',', '')
    ...
    insertvalue =  meigara_id
    insertvalue =  insertvalue + ", '" + syoriymd + "'"
    insertvalue =  insertvalue + ", " + str(ptskbn)
    insertvalue =  insertvalue + ", " + str(tmp_harimarine)
    insertvalue =  insertvalue + ", " + str(tmp_takane)
    insertvalue =  insertvalue + ", " + str(tmp_yasune)
    insertvalue =  insertvalue + ", " + str(tmp_owarine)
